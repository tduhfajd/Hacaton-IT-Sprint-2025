...
1. Создание обращения
   ├─ Web: Форма обращения
   └─ Telegram: /start → Категория → Описание
   
2. Анализ ИИ (автоматически)
   ├─ Определение приоритета
   ├─ Анализ тональности
   ├─ Поиск в базе знаний
   └─ Генерация рекомендованного ответа
   
3. Обработка оператором
   ├─ Открытие обращения: Новое → В работе
   ├─ Отправка ответа: чат закрывается автоматически
   └─ При новом сообщении: В работе → Новое (бейдж 🔴)
   
4. Завершение
   ├─ Вручную: кнопка "Завершить"
   ├─ Автоматически: 5 минут неактивности → Завершено
   └─ Уведомление в Telegram (если обращение из Telegram)
```

### Управление статусами

| Событие | Текущий статус | Новый статус | Действие |
|---------|---------------|--------------|----------|
| Оператор открыл чат | `new` | `in_progress` | Счётчик непрочитанных → 0 |
| Оператор отправил сообщение | `in_progress` | `in_progress` | Чат закрывается, счётчик → 0 |
| Гражданин отправил сообщение | `in_progress` | `new` | Счётчик +1, бейдж 🔴 |
| 5 минут неактивности | `in_progress` | `completed` | Уведомление в Telegram |
| Кнопка "Завершить" | `in_progress` | `completed` | Уведомление в Telegram |

### ИИ-анализ и генерация ответов

#### 1. Первичный анализ (при создании обращения)

```python
# Celery task: analyze_appeal
1. Получение описания обращения
...
```python
# Поиск по всей базе знаний (не только по категории)
1. Извлечение ключевых слов из вопроса
2. Поиск статей с совпадающими ключевыми словами
3. Ранжирование по релевантности (баллы)
4. Выбор наиболее релевантной статьи
5. Извлечение релевантной секции статьи
```

#### 3. Генерация ответа

```python
# Celery task: contextual_response
1. Получение истории диалога (последние N сообщений)
2. Получение релевантной статьи из БЗ
3. Фильтрация "пустых" сообщений:
   - Приветствия: "здравствуйте", "добрый день", etc.
   - Благодарности: "спасибо", "благодарю", etc.
   - Прощания: "до свидания", "пока", etc.
4. Если сообщение значимое:
   - Формирование промпта с контекстом
   - Отправка в GigaChat с полной статьёй
   - Получение человекоподобного ответа
5. Сохранение в ai_responses
```
